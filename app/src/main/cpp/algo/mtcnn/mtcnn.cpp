...
float MTCNN::IoU(float xmin, float ymin, float xmax, float ymax, float xmin_,
    ...
    if (is_iom) {
        float ov = s / min((xmax - xmin + 1) * (ymax - ymin + 1),
                           (xmax_ - xmin_ + 1) * (ymax_ - ymin_ + 1));
        return ov;
    } else {
        float ov = s / ((xmax - xmin + 1) * (ymax - ymin + 1) +
                        (xmax_ - xmin_ + 1) * (ymax_ - ymin_ + 1) - s);
        return ov;
    }
}
...
#pragma omp parallel for
        for (int32_t i = select_idx; i < num_bbox; i++) {
            ...
            switch (methodType) {
                case 'u':
                    if (static_cast<float>(area_intersect) / (area1 + area2 - area_intersect) >
                        thresh)
                        mask_merged[i] = 1;
                    break;
                case 'm':
...
vector<FaceInfo>
MTCNN::ProposalNet(const cv::Mat &img, int minSize, float threshold, float factor) {
    cv::Mat resized;
    int width = img.cols;
    int height = img.rows;
...
vector<FaceInfo>
MTCNN::NextStage(const cv::Mat &image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h,
    ...
    Blob<float> *input_layer = NULL;
    Blob<float> *confidence = NULL;
    Blob<float> *reg_box = NULL;
    Blob<float> *reg_landmark = NULL;
...
#pragma omp parallel for
    for (int n = 0; n < batch_size; ++n) {
        ...
        Mat roi = image(Rect(Point((int) box.xmin, (int) box.ymin),
                             Point((int) box.xmax, (int) box.ymax))).clone();
    ...
    const float *reg_data = reg_box->cpu_data();
    const float *landmark_data = NULL;
    if (reg_landmark) {
        ...
        if (confidence_data[2 * k + 1] >= threshold) {
            ...
            if (reg_landmark) {
                ...
                for (int i = 0; i < 5; ++i) {
                    ...
                    info.landmark[2 * i + 1] =
                            landmark_data[10 * k + 2 * i + 1] * h + info.bbox.ymin;
                }
            }
            res.push_back(info);
        }
...
vector<FaceInfo>
MTCNN::Detect(const cv::Mat &image, const int minSize, const float *threshold, const float factor,
              const int stage) {
    ...
    if (stage >= 2 && pnet_res.size() > 0) {
        ...
        int size = (int) ceil(1.f * num * step_size_float);
